...
export const Sidebar = ({ isOpen, onClose }) => {
  ...
  const roleTheme = isRecruiter
    ? {
        ...
        activeNav: 'border-pink-300 bg-gradient-to-r from-pink-50/80 via-white to-rose-50/60 text-pink-700 shadow-xs ring-1 ring-pink-400/20 font-extrabold',
        activeIcon: 'text-pink-600',
        activeChevron: 'text-pink-600',
        Icon: ShieldCheck,
      }
    : {
        label: 'Candidate Portal',
        badgeColor: 'text-blue-700 border-blue-200 bg-blue-50',
        activeNav: 'border-blue-300 bg-gradient-to-r from-blue-50/80 via-white to-indigo-50/60 text-blue-700 shadow-xs ring-1 ring-blue-400/20 font-extrabold',
  ...
  return (
    <>
      ...
      {/* --- Main Sidebar Container --- */}
      <aside
        className={`fixed top-16 bottom-0 left-0 z-40 flex w-64 flex-col justify-between border-r border-slate-200/80 bg-white/90 p-4 backdrop-blur-2xl transition-all duration-300 ease-in-out font-sans antialiased lg:translate-x-0 ${
      ...
      >
        <div className="space-y-6 overflow-y-auto pr-1 [scrollbar-width:none]">
          ...
          <div className="flex items-center justify-between px-2 pt-1 lg:hidden">
            ...
            <button
              type="button"
              onClick={onClose}
              className="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-slate-600 transition-colors hover:border-pink-300 hover:bg-pink-50 hover:text-pink-700 cursor-pointer shadow-2xs"
            >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between px-2">
              ...
              <span className={`inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-[10px] font-mono font-extrabold uppercase tracking-tight shadow-2xs ${roleTheme.badgeColor}`}>
                <roleTheme.Icon className="h-3 w-3 stroke-[2.5]" />
                Active
              </span>
            </div>
              ...
              {primaryLinks.map((link) => {
                ...
                return (
                  <NavLink
                    ...
                    onClick={onClose}
                    className={({ isActive }) =>
                      `group flex items-center justify-between rounded-2xl border px-3 py-2.5 text-xs font-bold transition-all duration-200 ${
                        isActive
                          ? `${roleTheme.activeNav}`
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        <div className="flex items-center gap-2.5">
                          <Icon className={`h-4 w-4 stroke-[2.2] transition-colors ${isActive ? roleTheme.activeIcon : 'text-slate-500 group-hover:text-blue-600'}`} />
                          <span>{link.name}</span>
                        </div>
                        <ChevronRight className={`h-3.5 w-3.5 stroke-[2.5] transition-all ${isActive ? `opacity-100 translate-x-0 ${roleTheme.activeChevron}` : 'opacity-0 -translate-x-1'}`} />
                      </>
                    )}
                  </NavLink>
                );
          ...
          <div className="space-y-2 border-t border-slate-200/80 pt-4">
            <p className="px-2 font-mono text-[11px] font-extrabold uppercase tracking-wider text-slate-500">
              Account & Support
            </p>
            <nav className="space-y-1">
              {secondaryLinks.map((link) => {
                ...
                return (
                  <NavLink
                    ...
                    onClick={onClose}
                    className={({ isActive }) =>
                      `group flex items-center justify-between rounded-2xl border px-3 py-2 text-xs font-bold transition-all duration-200 ${
                        isActive
                          ? 'border-indigo-300 bg-gradient-to-r from-indigo-50/80 via-white to-blue-50/60 text-indigo-700 shadow-xs ring-1 ring-indigo-400/20 font-extrabold'
                          : 'border-transparent text-slate-700 hover:border-slate-200 hover:bg-slate-50 hover:text-slate-950'
                      }`
                    }
                  >
                    {({ isActive }) => (
                      <>
                        <div className="flex items-center gap-2.5">
                          <Icon className={`h-4 w-4 stroke-[2.2] transition-colors ${isActive ? 'text-indigo-600' : 'text-slate-500 group-hover:text-indigo-600'}`} />
                          <span>{link.name}</span>
                        </div>
                        <ChevronRight className={`h-3.5 w-3.5 stroke-[2.5] transition-all ${isActive ? 'opacity-100 text-indigo-600' : 'opacity-0'}`} />
                      </>
                    )}
                  </NavLink>
                );
              ...
              <button
                ...
                className="group flex w-full items-center justify-between rounded-2xl border border-transparent px-3 py-2 text-xs font-bold text-slate-700 transition-all hover:border-slate-200 hover:bg-slate-50 hover:text-slate-950 cursor-pointer"
              >
                <div className="flex items-center gap-2.5">
                  <LogOut className="h-4 w-4 stroke-[2.2] text-slate-500 transition-colors group-hover:text-slate-800" />
                  <span>Sign Out</span>
                </div>
              </button>
              ...
              <button
                ...
                className="group flex w-full items-center justify-between rounded-2xl border border-transparent px-3 py-2 text-xs font-bold text-rose-600 transition-all hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700 cursor-pointer"
              >
                <div className="flex items-center gap-2.5">
                  <Trash2 className="h-4 w-4 stroke-[2.2] text-rose-500 transition-transform group-hover:scale-110" />
                  <span>Delete Account</span>
                </div>
              </button>
            </nav>
        ...
        <div className="relative mt-4 overflow-hidden rounded-2xl border border-blue-200/80 bg-gradient-to-br from-blue-50/60 via-white to-pink-50/40 p-3 shadow-xs backdrop-blur-xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="flex h-7 w-7 items-center justify-center rounded-xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-pink-500 p-1 shadow-xs">
                <Bot className="h-4 w-4 text-white stroke-[2.2]" />
              </div>
              <div>
                <span className="block text-xs font-black text-slate-950 leading-none">
                  Gemini 2.5 ATS
                </span>
                <span className="font-mono text-[9px] font-extrabold uppercase tracking-wider text-blue-700">Neural Engine</span>
              </div>
            </div>
            
            {/* Live Operational Radar Ping */}
